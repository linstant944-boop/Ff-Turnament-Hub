...
## Static QR Add Money Flow
Admin Panel includes Payment QR settings. Admin can upload one QR image and set minimum/maximum amount.
User enters an amount, sees the configured QR, scans it, and waits for verified payment.
The typed amount alone must never credit the wallet. Verified payment should be reconciled through Razorpay/webhook or an independently verified admin process.
